...
export default function NotificationsScreen({ onUnreadChange }: { onUnreadChange?: (count: number) => void }) {
  ...
  const getRelativeTime = (dateStr: string | undefined): string => {
    ...
    if (diffMins < 60)  return `${diffMins} ${diffMins > 1 ? t('minsLong') : t('minLong')}`;
    const diffHours = Math.floor(diffMins / 60);
    if (diffHours < 24) return `${diffHours} ${diffHours > 1 ? t('hoursLong') : t('hourLong')}`;
    const diffDays  = Math.floor(diffHours / 24);
    if (diffDays === 1) return t('yesterday');
    return `${diffDays} ${t('daysLong')}`;
  };


  // Build a translated notification message from type + data
  const getNotifMessage = (item: any): string => {
